...
else
{
	...
	$arOrder = Array("NAME" => "ASC");
	$arSelect = Array("ID", "NAME", "PROPERTY_STRUCTURE_TEAM_1", "PROPERTY_STRUCTURE_TEAM_2", "PROPERTY_AUTO_GOALS_TEAM_1", "PROPERTY_AUTO_GOALS_TEAM_2");
	...
	while($ob = $res->GetNext())
	{
		...
		$arR["AUTO_GOALS"] = array_count_values(array_merge($ob["PROPERTY_AUTO_GOALS_TEAM_1_VALUE"], $ob["PROPERTY_AUTO_GOALS_TEAM_2_VALUE"]));
		foreach ($arR["STRUCTURE"] as $value) {
			$arResult[$value]["GAMES"] ++;
			$arResult[$value]["AUTO_GOALS"] += $arR["AUTO_GOALS"][$value];
		}
		
	}
	function cmp_sort2($a, $b){
		if($a['AUTO_GOALS'] < $b['AUTO_GOALS']){
			return true;
		}
		elseif($a['AUTO_GOALS'] == $b['AUTO_GOALS']){
			if ($a["GAMES"] > $b["GAMES"]) {
				return true;
			}
			else{
