...
else
{
	...
	<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
		...
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			...
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				...
				<div class="form-group row <?//вырезаем NAME со страницы /personal/player/change.php 
				if($propertyID == "NAME" && dirname($_SERVER['REQUEST_URI'])=="/personal/player"){ echo 'd-none';}?>">
					...
					<div class="col-sm-10">
						...
						switch ($INPUT_TYPE):
							...
							case "G":
								?>
								<select class="form-control case-E section" name="PROPERTY[<?=$propertyID?>][]" <?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "Y"?"multiple":""?>>
										<?//if($arResult["PROPERTY_LIST_FULL"][$propertyID]["LINK_IBLOCK_ID"] == 1){
											if($arResult["PROPERTY_LIST_FULL"][$propertyID]["MULTIPLE"] == "N"){
												echo '<option></option>';
											}
											$fgc = file_get_contents("https://liga-alf.ru/api/tournament_sections.php");
								...
								<?/*<a href="javascript:" class="d-block" onclick="reset(<?=$propertyID?>)">сбросить</a>*/?>
	...
	<script>
		...
		// обработка сброса
		// function reset(code) {
		//   $("select[name='PROPERTY[" + code + "][]'] option:selected").removeAttr("selected");
		//   alert($("select[name='PROPERTY[" + code + "][]'] option:first").attr("selected", "").text("df"));
		//   //alert($("select[name='PROPERTY[" + code + "][]']").text(""));
		// }
	</script>
